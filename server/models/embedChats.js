...
const EmbedChats = {
  ...
  whereWithEmbedAndWorkspace: async function (
    ...
    try {
      ...
      console.error("EmbedChats.whereWithEmbedAndWorkspace failed, likely due to corrupted UTF-8 row. Falling back to robust fetch.", error.message);
      try {
        const ids = await prisma.embed_chats.findMany({
          where: clause,
          select: { id: true },
          ...(limit !== null ? { take: limit } : {}),
          ...(offset !== null ? { skip: offset } : {}),
        ...
        for (const { id } of ids) {
          try {
            const chat = await prisma.embed_chats.findUnique({
              where: { id },
              include: {
                embed_config: {
                  select: {
            ...
            console.error(`Skipping corrupted embed_chat id: ${id}`);
          }
        }
        return validChats;
      } catch (fallbackError) {
