...
const WorkspaceChats = {
  ...
  where: async function (
    ...
    try {
      ...
      console.error("WorkspaceChats.where failed, likely due to corrupted UTF-8 row. Falling back to robust fetch.", error.message);
      try {
        const ids = await prisma.workspace_chats.findMany({
          where: clause,
          select: { id: true },
          ...(limit !== null ? { take: limit } : {}),
          ...(offset !== null ? { skip: offset } : {}),
        ...
        for (const { id } of ids) {
          try {
            ...
            console.error(`Skipping corrupted workspace_chat id: ${id}`);
          }
        }
        return validChats;
      } catch (fallbackError) {
